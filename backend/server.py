...
def get_or_create_user(user_id: int, user_name: str = None, avatar: str = None):
    cursor.execute("SELECT balance, inventory, last_daily, user_name, avatar, total_games, max_win, total_profit, custom_title, frame_color, total_mines_x FROM users WHERE user_id = ?", (user_id,))
    row = cursor.fetchone()
    if row:
        b, inv, ld, un, av, tg, mw, tp, ct, fc, tmx = row[0], json.loads(row[1]), row[2], row[3], row[4], row[5], row[6], row[7], row[8] or '', row[9] or '', row[10] or 0.0
        if user_name or avatar:
            un = user_name or un
            av = avatar or av
            cursor.execute("UPDATE users SET user_name = ?, avatar = ? WHERE user_id = ?", (un, av, user_id))
            conn.commit()
        return b, inv, ld, un, av, tg, mw, tp, ct, fc, tmx
    
    un = user_name or f"Игрок #{str(user_id)[-4:]}"
    av = avatar or "👤"
    cursor.execute("INSERT INTO users (user_id, balance, inventory, last_daily, user_name, avatar, total_games, max_win, total_profit, custom_title, frame_color, total_mines_x) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, '', '', 0.0)",
            ...
            if current_mult >= game.target_crash:
                ...
                for uid, bet in list(game.active_bets.items()):
                    if bet["status"] == "playing" and bet["auto_cashout"] and current_mult >= bet["auto_cashout"]:
                        ...
                        b, inv, ld, un, av, tg, mw, tp, ct, fc, tmx = get_or_create_user(uid)
                        ...
                        if uid in game.connections:
                            await game.connections[uid].send_json({
                                ...
                                "stats": {"total_games": tg + 1, "max_win": max(mw, win_amount), "total_profit": tp + profit, "total_mines_x": tmx},
...
async def websocket_endpoint(websocket: WebSocket, user_id: int):
    ...
    b, inv, ld, u_name, u_ava, tg, mw, tp, ct, fc, tmx = get_or_create_user(user_id)
        ...
        while True:
            ...
            # --- 💣 МИНЫ (MINES) ---
                ...
                if m_game and m_game["status"] == "playing" and len(m_game["opened"]) > 0:
                    ...
                    await websocket.send_json({"type": "userData", "balance": new_bal, "inventory": inv, "stats": {"total_games": tg + 1, "max_win": max(mw, win_amount), "total_profit": tp + (win_amount - m_game["bet"]), "total_mines_x": new_tmx}, "custom_title": ct, "frame_color": fc})
