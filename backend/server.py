...
for col in [
    ...
    "total_profit INTEGER DEFAULT 0", "custom_title TEXT DEFAULT ''", "frame_color TEXT DEFAULT ''"
]:
    try:
...
def get_or_create_user(user_id: int, user_name: str = None, avatar: str = None):
    cursor.execute("SELECT balance, inventory, last_daily, user_name, avatar, total_games, max_win, total_profit, custom_title, frame_color FROM users WHERE user_id = ?", (user_id,))
    row = cursor.fetchone()
    if row:
        b, inv, ld, un, av, tg, mw, tp, ct, fc = row[0], json.loads(row[1]), row[2], row[3], row[4], row[5], row[6], row[7], row[8] or '', row[9] or ''
        if user_name or avatar:
            un = user_name or un
            av = avatar or av
            cursor.execute("UPDATE users SET user_name = ?, avatar = ? WHERE user_id = ?", (un, av, user_id))
            conn.commit()
        return b, inv, ld, un, av, tg, mw, tp, ct, fc
    
    un = user_name or f"Игрок #{str(user_id)[-4:]}"
    av = avatar or "👤"
    cursor.execute("INSERT INTO users (user_id, balance, inventory, last_daily, user_name, avatar, total_games, max_win, total_profit, custom_title, frame_color) VALUES (?, ?, ?, ?, ?, ?, 0, 0, 0, '', '')",
                   (user_id, 10000000000, '[]', 0, un, av))
    conn.commit()
    return 10000000000, [], 0, un, av, 0, 0, 0, '', ''

def update_user_data(user_id: int, balance: int = None, inventory: list = None, last_daily: int = None, user_name: str = None, avatar: str = None, games_add: int = 0, win_amount: int = 0, profit_add: int = 0, custom_title: str = None, frame_color: str = None):
    cursor.execute("SELECT balance, total_games, max_win, total_profit, custom_title, frame_color FROM users WHERE user_id = ?", (user_id,))
    row = cursor.fetchone()
    if not row:
        return
    cur_bal, cur_tg, cur_mw, cur_tp, cur_ct, cur_fc = row[0], row[1] or 0, row[2] or 0, row[3] or 0, row[4] or '', row[5] or ''
    ...
    cursor.execute("""
        UPDATE users 
        SET balance = ?, total_games = ?, max_win = ?, total_profit = ?, custom_title = ?, frame_color = ?
        WHERE user_id = ?
    """, (new_bal, new_tg, new_mw, new_tp, new_ct, new_fc, user_id))
            ...
            if current_mult >= game.target_crash:
                ...
                for uid, bet in list(game.active_bets.items()):
                    if bet["status"] == "playing" and bet["auto_cashout"] and current_mult >= bet["auto_cashout"]:
                        ...
                        b, inv, ld, un, av, tg, mw, tp, ct, fc = get_or_create_user(uid)
    ...
    while True:
        if game.jackpot_state == "idle":
            if len(game.jackpot_bets) >= 2:
                ...
                b, inv, ld, un, av, tg, mw, tp, ct, fc = get_or_create_user(winner_id)
                new_bal = b + total_pot
...
async def websocket_endpoint(websocket: WebSocket, user_id: int):
    ...
    b, inv, ld, u_name, u_ava, tg, mw, tp, ct, fc = get_or_create_user(user_id)
        ...
        while True:
            ...
            if action == "bet":
                ...
                if room and user_id != room["creator_id"] and b >= room["amount"]:
                    ...
                    wb, winv, wld, wun, wav, wtg, wmw, wtp, wct, wfc = get_or_create_user(winner_id)
                ...
                if bet > 0 and b >= bet:
                    ...
                    for _ in range(8):
                        safe = [0, 1, 2]
                        safe.remove(random.randint(0, 2))
                        floors.append(safe)

                    game.tower_games[user_id] = {"bet": bet, "floor": 0, "floors": floors, "status": "playing"}
                    
                    await websocket.send_json({"type": "userData", "balance": new_bal, "inventory": inv, "stats": {"total_games": tg, "max_win": mw, "total_profit": tp}, "custom_title": ct, "frame_color": fc})
                    await websocket.send_json({"type": "tower_state", "status": "playing", "floor": 0, "mult": 1.0, "win": 0})

            elif action == "tower_step":
                door = data.get("door")
                t_game = game.tower_games.get(user_id)
                if t_game and t_game["status"] == "playing":
                    cur_floor = t_game["floor"]
                    safe_doors = t_game["floors"][cur_floor]
                    
                    if door in safe_doors:
                        t_game["floor"] += 1
                        next_floor = t_game["floor"]
                        mults = [1.4, 2.1, 3.2, 5.0, 8.0, 14.0, 25.0, 50.0]
                        mult = mults[next_floor - 1]
                        win = int(t_game["bet"] * mult)
                        
                        if next_floor == 8:
                            b, inv, ld, _, _, tg, mw, tp, ct, fc = get_or_create_user(user_id)
                            update_user_data(user_id, balance=b + win, games_add=1, win_amount=win, profit_add=win - t_game["bet"])
                            t_game["status"] = "cashed_out"
                            await websocket.send_json({"type": "userData", "balance": b + win, "inventory": inv, "stats": {"total_games": tg + 1, "max_win": max(mw, win), "total_profit": tp + (win - t_game["bet"])}, "custom_title": ct, "frame_color": fc})
                            await websocket.send_json({"type": "tower_state", "status": "cashed_out", "floor": 8, "mult": mult, "win": win})
                        else:
                            await websocket.send_json({"type": "tower_state", "status": "playing", "floor": next_floor, "mult": mult, "win": win})
                    else:
                        t_game["status"] = "crashed"
                        update_user_data(user_id, games_add=1, profit_add=-t_game["bet"])
                        await websocket.send_json({"type": "tower_state", "status": "crashed", "floor": cur_floor, "mult": 0, "win": 0})

            elif action == "tower_cashout":
                t_game = game.tower_games.get(user_id)
                if t_game and t_game["status"] == "playing" and t_game["floor"] > 0:
                    mults = [1.4, 2.1, 3.2, 5.0, 8.0, 14.0, 25.0, 50.0]
                    mult = mults[t_game["floor"] - 1]
                    win_amount = int(t_game["bet"] * mult)
                    
                    b, inv, ld, _, _, tg, mw, tp, ct, fc = get_or_create_user(user_id)
                    new_bal = b + win_amount
                    update_user_data(user_id, balance=new_bal, games_add=1, win_amount=win_amount, profit_add=win_amount - t_game["bet"])
                    t_game["status"] = "cashed_out"
                    
                    await websocket.send_json({"type": "userData", "balance": new_bal, "inventory": inv, "stats": {"total_games": tg + 1, "max_win": max(mw, win_amount), "total_profit": tp + (win_amount - t_game["bet"])}, "custom_title": ct, "frame_color": fc})
                    await websocket.send_json({"type": "tower_state", "status": "cashed_out", "floor": t_game["floor"], "mult": mult, "win": win_amount})
                ...
                if bet > 0 and b >= bet and 1 <= mines_count <= 24:
                    new_bal = b - bet
                    update_user_data(user_id, balance=new_bal)
                    grid = ["gem"] * 25
                ...
                if m_game and m_game["status"] == "playing" and len(m_game["opened"]) > 0:
                    ...
                    b, inv, ld, _, _, tg, mw, tp, ct, fc = get_or_create_user(user_id)
                    new_bal = b + win_amount
                    update_user_data(user_id, balance=new_bal, games_add=1, win_amount=win_amount, profit_add=win_amount - m_game["bet"])
                    m_game["status"] = "cashed_out"
                    
                    await websocket.send_json({"type": "userData", "balance": new_bal, "inventory": inv, "stats": {"total_games": tg + 1, "max_win": max(mw, win_amount), "total_profit": tp + (win_amount - m_game["bet"])}, "custom_title": ct, "frame_color": fc})
                ...
                if b >= cost:
                    new_bal = b - cost
                    if item_type == 'title':
                        update_user_data(user_id, balance=new_bal, custom_title=val)
                        ct = val
                    elif item_type == 'frame':
                        update_user_data(user_id, balance=new_bal, frame_color=val)
                        fc = val
                        
                    await websocket.send_json({"type": "userData", "balance": new_bal, "inventory": inv, "stats": {"total_games": tg, "max_win": mw, "total_profit": tp}, "custom_title": ct, "frame_color": fc})
                    await websocket.send_json({"type": "notify", "msg": "🎉 Успешно куплено!"})
