...
async def create_star_invoice_link(stars_amount: int, user_id: str):
    ...
    payload = {
        "title": f"Пополнение {stars_amount} Stars",
        "description": f"Пополнение баланса в DOOM GIFT",
    ...
    try:
        while True:
            ...
            if action == "buy_stars_invoice":
                ...
                link = await create_star_invoice_link(amount, uid)
                if link:
                    await websocket.send_json({"type": "open_invoice", "url": link, "stars": amount})
                else:
                    await websocket.send_json({"type": "notify", "msg": "❌ Ошибка создания счета! Проверьте BOT_TOKEN в настройках Render."})
